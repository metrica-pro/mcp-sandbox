...
async def main():
    ...
    async with sse_client("http://localhost:8181/sse") as (read, write):
        async with ClientSession(read, write) as session:
            ...
            print(
                "=== execute_code(language='javascript', code='console.log(6*7)') ==="
            )
            result = await session.call_tool(
                "execute_code",
            ...
            print(
                "=== execute_code(language='python', code='import math; print(math.factorial(10))') ==="
            )
            result = await session.call_tool(
                "execute_code",
                arguments={
